...
-(void) handleDragStartedInDstAtPoint:(CGPoint) point{
    ...
    UIView *sumView = [self.superview viewWithTag:555];
    sumView.hidden = YES;
    CGRect frame = sumView.frame;
    UIView *view = [[UIView alloc] initWithFrame:frame];
    view.autoresizingMask = UIViewAutoresizingFlexibleBottomMargin;
    view.tag = 666;
    UIColor *background = [[UIColor alloc] initWithPatternImage:[UIImage imageNamed:@"bg_gradient_red"]];
    [view setBackgroundColor:background];
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, frame.size.height)];
    label.text = @"Drag here to remove";
...
-(void) handleDragFromDstStoppedAtPoint:(CGPoint) point{
    ...
    CGRect rect = [self.superview convertRect:self.dstView.frame fromView:self.dstView];
    BOOL inside = CGRectContainsPoint(rect, point);
    
    if(self.delegate && [self.delegate respondsToSelector:@selector(droppedOutsideAtPoint:fromDstIndexPath:)] && !inside){
        if(CGRectContainsPoint([sumView frame], point)) {
    ...
    UIView *view = [self.superview viewWithTag:666];
